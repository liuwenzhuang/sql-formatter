...
describe('HqlFormatter', () => {
  const format = (query, cfg = {}) => sqlFormatter.format(query, { ...cfg, language: 'hql' });
  ...
  it('does not ad spaces around slash', () => {
    const result = format('ADD jar hdfs://user/test.jar;');
    expect(result).toBe(dedent`
      ADD
        jar hdfs://user/test.jar;
    `);
  });
});
